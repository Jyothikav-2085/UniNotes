...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    if (!isGoogleConfigured) {
      console.warn('Google Client ID is not properly configured. Google login will not work.');
      console.warn('Please set VITE_GOOGLE_CLIENT_ID in your .env file with a valid Google OAuth 2.0 Client ID.');
    }
  }, [searchParams]);
  ...
  const handleGoogleSuccess = async (response: GoogleLoginResponse | GoogleLoginResponseOffline) => {
    ...
    console.log('Google login response type:', typeof response);
    console.log('Google login response keys:', Object.keys(response));
    ...
    if ('profileObj' in response) {
      try {
        ...
        const backendResponse = await fetch(`${API_BASE_URL}${API_ENDPOINTS.GOOGLE_AUTH}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        console.log('Google auth backend response:', backendResponse.status, data);
        ...
        if (backendResponse.ok) {
          ...
          toast({
            ...
            description: data.error || 'Failed to authenticate with Google',
            variant: "destructive",
          });
        }
      } catch (error) {
      ...
      console.error('Google login response does not contain profileObj:', response);
      toast({
        title: "Google Sign In Failed",
  ...
  const handleGoogleFailure = (error: { error: string; details?: string }) => {
    console.error('Google login failed:', error);
    console.error('Google login failure details:', {
      error: error.error,
                  ...
                  render={renderProps => {
                    ...
                    if (!isGoogleConfigured) {
                      return (
                        <Button
                          variant="outline"
                          className="w-full glass border-2 hover:border-primary/50 transition-all py-6 text-base bg-red-500/10"
                          disabled={true}
                        >
                    ...
                    return (
                      <Button
                        ...
                        variant="outline"
                        className="w-full glass border-2 hover:border-primary/50 transition-all py-6 text-base"
                      >
                        <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                          <path
                            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
